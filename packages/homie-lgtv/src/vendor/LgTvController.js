...
const WEBOS_URI_TURN_OFF_SCREEN =
  "ssap://com.webos.service.tv.power/turnOffScreen"; // params: standbyMode (active or passive[passive cannot turn screen back on])
const WEBOS_URI_TURN_ON_SCREEN =
  "ssap://com.webos.service.tv.power/turnOnScreen"; // params: standbyMode (active or passive[passive cannot turn screen back on])
const WEBOS_URI_TURN_OFF_SCREEN_ALT =
  "ssap://com.webos.service.tvpower/power/turnOffScreen"; // alternative version, probably for webOS5+ TVs, accepts the same params as above
const WEBOS_URI_TURN_ON_SCREEN_ALT =
  "ssap://com.webos.service.tvpower/power/turnOnScreen"; // alternative version, probably for webOS5+ TVs, accepts the same params as above
...
const WEBOS_URI_REMOTE_POINTER_SOCKET_INPUT =
...
class LgTvController extends EventEmitter {
  ...
  setupLgtvInstance() {
    ...
    this.lgtv.on("prompt", () => {
      this.logInfo(
        "Prompt for confirmation. Please accept the pairing request on the TV!",
      );
      this.connected = false;
    });
  ...
  setupTvConnection() {
    this.getTvInformation().then(() => {
      ...
      let webOsVer = this.getWebOsVersion() > 0
        ? this.getWebOsVersion().toFixed(1)
        : "Unknown";
      ...
      this.subscribeToServices().then(() => {
        ...
        this.connectToPointerInputSocket().then(() => {
          ...
          this.logDebug(error);
          this.logInfo(
            "Setup finished with remote input socket error! Remote control emulation will not be possible!",
          );
          this.emit(Events.SETUP_FINISHED);
        });
      });
  ...
  async getTvInformation() {
    ...
    tvInfoPromises.push(
      this.getLaunchPoints().then((res) => {
        if (res && res.launchPoints && Array.isArray(res.launchPoints)) {
          ...
          this.logDebug(
            "Error while retrieving the launch point list \n" +
    ...
    tvInfoPromises.push(
      this.getExternalInputs().then((res) => {
        if (res && res.devices && Array.isArray(res.devices)) {
          ...
          this.logDebug(
            "Error while retrieving the external input list \n" +
              JSON.stringify(res, null, 2),
          );
        }
      }),
    ...
    tvInfoPromises.push(
      this.getAllApps().then((res) => {
        if (res && res.apps && Array.isArray(res.apps)) {
          ...
          this.logDebug(
            "Error while retrieving all apps list \n" +
              JSON.stringify(res, null, 2),
          );
        }
      }),
  ...
  async subscribeToPowerStatusChange() {
    return this.tvSubscribe(WEBOS_URI_POWER_STATE, null, (err, res) => {
      ...
      let statusPowerOnReason = res && res.powerOnReason
        ? res.powerOnReason
        : null;
      let powerState = "";
      ...
      this.logDebug(
        `Failed to subscribe to the Power State service. The configured TV most probably does not support that service. Falling back to tcp ping!`,
      );
  ...
  async subscribeToAudioStatusChange() {
    return this.tvSubscribe(WEBOS_URI_AUDIO_STATUS, null, (err, res) => {
      this.logDebug(
        `Audio status changed. Current volume: ${res.volume}, Muted: ${
          res.mute ? "Yes" : "No"
        }`,
      );
  ...
  async subscribeToForegroundAppChange() {
    return this.tvSubscribe(WEBOS_URI_FOREGROUND_APP_INFO, null, (err, res) => {
      if (res && res.appId && res.appId.length > 0) {
        ...
        if (
          this.foregroundApp.appId === WEBOS_LIVE_TV_APP_ID &&
          !this.currentLiveTvChannel
        ) {
          setTimeout(
            () => this.subscribeToChannelChange(),
            LIVE_TV_SUBSCRIBE_DELAY,
          ); // short delay as it for some reason does not work instantly
        }
    ...
    return this.tvSubscribe(WEBOS_URI_CURRENT_CHANNEL, null, (err, res) => {
      if (!res || err || res.errorCode) {
        ...
        if (this.getCurrentLiveTvChannelId() !== res.channelId) {
          this.logInfo(
            `Channel changed. Current channel: ${res.channelNumber}, ${res.channelName}, channelId: ${res.channelId}`,
          );
      ...
      this.currentLiveTvChannel = null;
      this.logDebug(
        `Failed to subscribe to the channel service. It seems Live TV is not running, will retry after switching to Live TV.`,
      );
    });
  }
  ...
  async subscribeToSoundOutputChange() {
    return this.tvSubscribe(WEBOS_URI_SOUND_OUPUT, null, (err, res) => {
      if (this.activeSoundOutput !== res.soundOutput) {
        this.logInfo(
          `Sound output changed. Current sound output: ${res.soundOutput}`,
        );
    ...
    return this.tvSubscribe(WEBOS_URI_LAUNCH_POINTS, null, (err, res) => {
      if (res && res.launchPoints && Array.isArray(res.launchPoints)) {
        ...
        if (res.change === "added" && res.id !== WEBOS_AIRPLAY_APP_ID) {
          this.logDebug(
            `New app installed on the TV! app: ${res.title} appId: ${res.id}`,
          );
        ...
        if (res.change === "removed" && res.id !== WEBOS_AIRPLAY_APP_ID) {
          this.logDebug(
            `App removed from TV! app: ${res.title} appId: ${res.id}`,
          );
  ...
  async subscribeToPictureSettingsChange() {
    ...
    return this.tvSubscribe(WEBOS_URI_SYSTEM_SETTINGS, payload, (err, res) => {
      if (res && res.settings) {
        ...
        this.logDebug(
          `Picture settings changed. Current picture settings: ${
            JSON.stringify(this.pictureSettings)
          }`,
        );
  ...
  async subscribeToSoundSettingsChange() {
    ...
    return this.tvSubscribe(WEBOS_URI_SYSTEM_SETTINGS, payload, (err, res) => {
      if (res && res.settings) {
        ...
        this.logDebug(
          `Sound settings changed. Current sound settings: ${
  ...
  async connectToPointerInputSocket() {
    ...
    return new Promise((resolve, reject) => {
      this.lgtv.getSocket(
        WEBOS_URI_REMOTE_POINTER_SOCKET_INPUT,
        (err, sock) => {
          if (!err) {
  ...
  getCurrentLiveTvChannelNumber() {
    if (
      this.isLiveTvActive() && this.currentLiveTvChannel &&
      this.currentLiveTvChannel.channelNumber
    ) {
  ...
  getCurrentLiveTvChannelId() {
    if (
      this.isLiveTvActive() && this.currentLiveTvChannel &&
      this.currentLiveTvChannel.channelId
    ) {
  ...
  getCurrentLiveTvChannelName() {
    if (
      this.isLiveTvActive() && this.currentLiveTvChannel &&
      this.currentLiveTvChannel.channelName
    ) {
  ...
  getForegroundAppAppId() {
    if (
      this.foregroundApp && this.foregroundApp.appId &&
      this.foregroundApp.appId.length > 0
    ) {
      ...
      if (this.connected) {
        return new Promise((resolve, reject) => {
          ...
          this.lgtv.request(methodUri, payload, (err, res) => {
            if (
              !res || err || res.errorCode || res.errorText || !res.returnValue
            ) {
              if (res && Object.keys(res).length > 0) {
                reject(
                  `Request failed. Error text: ${res.errorText}, Error code: ${res.errorCode}`,
                );
              } else {
                reject(
                  `Unknown error while requesting ${method} from service ${service}`,
                );
              }
            } else {
        ...
        this.logDebug(
          `TV not connected - cannot fire ${method} from service ${service}!`,
        );
      }
    }
  }
      ...
      if (this.connected) {
        ...
        let tvSubscribePromise = new Promise((resolve, reject) => {
          ...
          this.lgtv.subscribe(methodUri, payload, (err, res) => {
            if (
              !res || Object.keys(res).length == 0 || err || res.errorCode ||
              res.errorText
            ) {
              if (res && Object.keys(res).length > 0) {
                reject(
                  `Could not subscribe to ${method} from service ${service}. Error text: ${res.errorText}, Error code: ${res.errorCode}`,
                );
              } else {
                reject(
                  `Unknown error while subscribing to ${method} from service ${service}`,
                );
              }
            } else {
              this.logDeepDebugTvResponse(res); // log the Tv response
        ...
        let timeoutPromise = new Promise((resolve, reject) => {
          setTimeout(
            () =>
              reject(
                `Timed out while subscribing to ${method} from service ${service}`,
              ),
            AUTO_PROMISE_RESOLVE_TIMEOUT * 2,
          );
        });
        ...
        this.logDebug(
          `TV not connected - cannot subscribe to ${method} from service ${service}!`,
        );
        resolve();
      }
    }
  ...
  async turnOn() {
    wol.wake(this.mac, {
      ...
      this.logDebug(
        `Wake on LAN callback - sent magic packet, trying to connect to tv...`,
      );
      this.logDeepDebug(
        `Trying to wake up TV! Mac: ${this.mac}, BroadcastAdr: ${this.broadcastAdr}`,
      );
      if (this.connected === false) {
        this.connect();
      }
    ...
    return new Promise((resolve, reject) => {
      setTimeout(
        resolve,
        Math.max(AUTO_PROMISE_RESOLVE_TIMEOUT * 6, this.reconnect),
      ); //backup, turn on takes longer to process so auto resolve after 30 seconds or at least the choosen reconnect time
  ...
  async setVolumeLevel(volumeLevel) {
    ...
    return this.tvRequest(
      WEBOS_URI_SET_VOLUME,
      payload,
      `Volume: ${volumeLevel}, Volume limit: ${this.volumeLimit}`,
    );
  }
  ...
  changeSoundOutput(soundOutput) {
    ...
    this.tvRequest(
      WEBOS_URI_CHANGE_SOUND_OUTPUT,
      payload,
      `Changing sound output to: ${soundOutput}`,
    );
  }
  ...
  openLiveTvChannel(channelNumber, channelId) {
    ...
    this.tvRequest(
      WEBOS_URI_OPEN_CHANNEL,
      payload,
      `Channel number: ${channelNumber}, Channel Id: ${channelId}`,
    );
  }
  ...
  switchInput(inputId) {
    ...
    this.tvRequest(
      WEBOS_URI_SWITCH_INPUT,
      payload,
      `Changing input to: ${inputId}`,
    );
  }
  ...
  turnOffTvScreen() {
    ...
    let endpointOff = this.getWebOsVersion() >= 5
      ? WEBOS_URI_TURN_OFF_SCREEN_ALT
      : WEBOS_URI_TURN_OFF_SCREEN;
    let payload = {
      standbyMode: "active",
    };
  ...
  turnOnTvScreen() {
    // possible standbyMode values 'active' and 'passive', passive stay on even when TV is turned off, do not know yet how to get out of that mode - currentyl need to pull tv plug
    let endpointOn = this.getWebOsVersion() >= 5
      ? WEBOS_URI_TURN_ON_SCREEN_ALT
      : WEBOS_URI_TURN_ON_SCREEN;
    let payload = {
      standbyMode: "active",
    };
  ...
  async launchApp(appId, params = {}) {
    if (appId && appId.length > 0) {
      ...
      return this.tvRequest(
        WEBOS_URI_LAUNCH_APP,
        payload,
        `AppId: ${appId}, Params: ${JSON.stringify(params)}`,
      );
    } else {
      this.logDebug(
        `Missing or invalid appId. Cannot launch app on the tv! AppId: ${appId}, Params: ${
  ...
  async openApp(appId) {
    if (appId && appId.length > 0) {
      ...
      this.logDebug(
        `Missing or invalid appId. Cannot open app on the tv! AppId: ${appId}`,
      );
    }
  }
  ...
  closeApp(appId) {
    if (appId && appId.length > 0) {
      ...
      this.logDebug(
        `Missing or invalid appId. Cannot close app on the tv! AppId: ${appId}`,
      );
    }
  }
  ...
  openYouTubeVideoId(youtubeId) {
    if (youtubeId && youtubeId.length > 0) {
      ...
      this.logDebug(
        `Missing or invalid youtube content id. Cannot open youtube video! YouTube video id: ${youtubeId}`,
      );
    }
  }
  ...
  openNetflixContent(contentId) {
    if (contentId && contentId.length > 0) {
      ...
      this.logDebug(
        `Missing or invalid netflix content id. Cannot open netflix content! Netflix content id: ${contentId}`,
      );
    }
  }
  ...
  openAmazonContent(contentId) {
    if (contentId && contentId.length > 0) {
      ...
      this.logDebug(
        `Missing or invalid amazon content id. Cannot open amazon content! Amazon content id: ${contentId}`,
      );
    }
  }
  ...
  async openToast(message, iconData, iconExtension, onClick) {
    ...
    return this.tvRequest(
      WEBOS_URI_CREATE_TOAST,
      payload,
      `Message: ${message}, Icon extension: ${iconExtension}, onClick: ${onClick}`,
    );
  }
  ...
  async openAlert(
    ...
    return this.tvRequest(
      WEBOS_URI_CREATE_ALERT,
      payload,
      `Title: ${title}, Message: ${message}, Type: ${type}, Modal: ${modal}`,
    );
  }
  ...
  sendRemoteInputSocketCommand(cmd) {
    if (this.connected && this.pointerInputSocket) {
      ...
      this.logDebug(
        `Remote input socket - tv not connected or remote input socket not established!`,
      );
    }
  }
  ...
  checkTvPowerState() {
    ...
    if (
      this.powerState.state && this.powerState.state === "Suspend" &&
      !this.powerState.processing
    ) {
      return "Off";
    }
    ...
    if (
      this.powerState.processing &&
      this.powerState.processing === "Request Suspend"
    ) {
      return "Off";
    }

    if (
      this.powerState.state && this.powerState.state === "Active Standby" &&
      !this.powerState.processing
    ) {
    ...
    if (
      this.powerState.state && this.powerState.state === "Screen Saver" &&
      !this.powerState.processing
    ) {
      return "Screen Saver";
    }
    ...
    if (
      this.powerState.state && this.powerState.state === "Screen Off" &&
      this.powerState.processing && this.powerState.processing === "Screen On"
    ) {
      return "Screen On";
    }
    ...
    if (
      this.powerState.state && this.powerState.state === "Active" &&
      !this.powerState.processing
    ) {
      return "On";
    }
  ...
  checkBasicInputs() {
    ...
    if (
      this.getLaunchPointsList().some((app) =>
        app.appId === WEBOS_LIVE_TV_APP_ID
      ) === false
    ) {
      this.logDeepDebug(
        `Live TV app is missing in the launch points list, adding it manually!`,
      );
      let liveTvApp = this.getAllAppsList().find((app) =>
        app.appId === WEBOS_LIVE_TV_APP_ID
      );
      if (liveTvApp) {
        let newApp = {};
    ...
    this.getExternalInputList().forEach((externalInput, i) => {
      if (
        this.getLaunchPointsList().some((app) =>
          app.appId === externalInput.appId
        ) === false
      ) {
        this.logDeepDebug(
          `Input ${externalInput.appId} is missing in the launch points list, adding it manually!`,
        );
        let newApp = {};
  ...
  webOs2xStateFallback() {
    ...
    // start the polling
    if (!this.webOs2xFallbackCheckAliveInterval) {
      this.webOs2xFallbackCheckAliveInterval = setInterval(
        this.webOs2xStateFallback.bind(this),
        Math.min(4000, this.reconnect / 2),
      ); // at least 2 seconds polling
    }
